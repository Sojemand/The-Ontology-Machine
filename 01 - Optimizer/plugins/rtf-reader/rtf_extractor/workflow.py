"""Workflow surface for the rtf-reader extractor."""
...
from .types import RtfProjection, RtfStageError
...
def selftest() -> dict[str, Any]:
    try:
        adapter.ensure_striprtf()
    except RtfStageError as exc:
        return {"status": "error", "version": _VERSION, "error": exc.detail}
    return {"status": "ok", "version": _VERSION}


def _success_envelope(projection: RtfProjection, start: int) -> dict[str, Any]:
    return {
        "status": "success",
