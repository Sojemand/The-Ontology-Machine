"""Pure output naming policy for the file processor."""
...
def iter_output_candidates(extracts_dir: Path, slug: str, page_suffix: str, short_hash: str):
    seen: set[Path] = set()
    first = extracts_dir / budget_output_name(extracts_dir, f"{slug}{page_suffix}.raw.json", reserved=len(_OUTPUT_CLAIM_SUFFIX))
    if first not in seen:
        seen.add(first)
        yield first
    collision_slug = f"{slug}.{short_hash}"
    second = extracts_dir / budget_output_name(extracts_dir, f"{collision_slug}{page_suffix}.raw.json", reserved=len(_OUTPUT_CLAIM_SUFFIX))
