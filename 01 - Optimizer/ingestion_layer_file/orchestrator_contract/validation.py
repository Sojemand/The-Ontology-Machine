...
from .types import HEALTHCHECK_DEPENDENCIES

_ALLOWED_HEALTHCHECK_DEPENDENCIES = frozenset(HEALTHCHECK_DEPENDENCIES)
MAX_WORKER_COUNT = 8


def require_object_payload(payload: object) -> dict:
...
def require_pdf_source_path(payload: dict) -> Path:
    source_path = require_source_path(payload)
    if source_path.suffix.lower() != ".pdf":
        raise ValueError(f"classify_document akzeptiert nur PDF-Dateien: {source_path}")
    return source_path
...
def require_page_images_dir(payload: dict) -> Path:  # pragma: no cover - legacy alias
    return require_page_assets_dir(payload)
...
def parse_healthcheck_request(payload: dict) -> tuple[str, tuple[str, ...] | None]:
    scope = str(payload.get("scope", "")).strip()
    raw_dependencies = payload.get("required_dependencies")
    if raw_dependencies is None:
        return scope, None
    if not isinstance(raw_dependencies, list):
        raise ValueError("required_dependencies muss eine Liste von Strings sein.")
    ...
    for item in raw_dependencies:
        ...
        name = item.strip()
        if not name:
            raise ValueError("required_dependencies enthaelt einen leeren Eintrag.")
        if name not in _ALLOWED_HEALTHCHECK_DEPENDENCIES:
            raise ValueError(f"required_dependencies enthaelt unbekannte Abhaengigkeit: {name}")
        if name not in dependencies:
            dependencies.append(name)
    return scope, tuple(dependencies)
