    ...
    *,
    ...
    try:
        if mode == "single":
            ...
            result, total = _run_batch(payload, input_root, session_root, output_root, config, plugin_mgr, filters, processor_cls, layout.state_dir if validation.use_processed_hashes(payload) else None)
        if debug_support.cancel_requested(session_root):
            return _cancelled(session_root, "Debuglauf abgebrochen")
...
def _run_single(payload, session_root, output_root, config, plugin_mgr, processor_cls) -> str:
    ...
    _run_with_cancel_watch(session_root, processor, lambda: processor.process_single(source_path, write_output=True, raw_output_path=raw_output_path, page_assets_dir=page_assets_dir, logical_source_path=logical_source_path))
...
def _run_batch(payload, input_root, session_root, output_root, config, plugin_mgr, filters, processor_cls, state_dir):
    ...
    processor = processor_cls(config, plugin_mgr, catalog, filters, output_root, callback=lambda report: _on_report(session_root, report, total))
...
def _counters(outputs: dict[str, list[str]]) -> dict[str, int]:
    return {
        ...
        "page_assets_written": len(outputs.get("page_assets", [])),
    }
