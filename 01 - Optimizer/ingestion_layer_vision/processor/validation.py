"""Hard validation boundaries for processor inputs and routing."""
...
def result_error_detail(result: ExtractResult, default: str) -> str:
    raw_errors = result.errors
    values = [raw_errors] if isinstance(raw_errors, str) else raw_errors if isinstance(raw_errors, list) else [raw_errors] if raw_errors else []
    details = [str(value).strip() for value in values if str(value).strip()]
...
def ensure_file_size(size: int, max_file_size_mb: int) -> None:
    ...
    if size > max_bytes:
        raise FileTooLargeError(f"Datei zu gross: {size} bytes (max {max_bytes})")
...
def ensure_success_result(plugin_name: str, result: ExtractResult) -> None:
    if result.status != "success":
        raise PluginError(plugin_name, result_error_detail(result, "Plugin lieferte Fehlerstatus ohne Details"))
...
def validate_explicit_single_file_targets(
    *,
    ...
    normalized_logical_path = normalize_logical_source_path(logical_source_path)
    ...
    return normalized_logical_path
