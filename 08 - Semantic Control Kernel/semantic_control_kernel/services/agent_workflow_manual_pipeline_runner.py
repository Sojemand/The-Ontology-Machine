...
from semantic_control_kernel.workflows.pipeline_run.interaction_port import ManualPipelineInteractionPort
from semantic_control_kernel.workflows.pipeline_run.manual import manual_pipeline_run


def run_manual_pipeline_agent_workflow(
    *,
    ...
    interaction_port = ManualPipelineInteractionPort(state_paths, runtime=runtime)
    collected = interaction_port.collect_pipeline_inputs(
    ...
    if collected is None:
        ...
        sync_manual_pipeline_workflow_run(execution.to_dict(), state_paths=state_paths)
        return execution
    ...
    execution = manual_pipeline_run(
        ...
        input_files=collected.input_files,
        workflow_run_id=resolved_workflow_run_id,
...
def sync_manual_pipeline_workflow_run(execution: Mapping[str, Any], *, state_paths: StatePaths) -> None:
...
def _waiting_execution(
    state_paths: StatePaths,
    workflow_run_id: str,
    interaction_port: ManualPipelineInteractionPort,
) -> PipelineRunExecution:
    request = interaction_port.pending_pipeline_request(workflow_run_id)
    if request is None:
        return _blocked_execution(
            ...
            PipelineRunBlocker(
                ...
                user_visible_summary="Manual pipeline run target and Input evidence must be collected through Kernel/UI state.",
            ),
        )
    summary = str(request.payload.get("user_visible_summary") or "The Kernel is waiting for manual pipeline run input.")
    ...
    execution.final_state = "awaiting_manual_pipeline_interaction"
    execution.blocked_step_id = "manual_pipeline_collect_interaction"
