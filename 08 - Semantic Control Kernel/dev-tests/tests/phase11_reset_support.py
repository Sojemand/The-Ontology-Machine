...
from test_phase11_fakes import FakeCorpusAdapter, ok_result
...
def state_paths_for(tmp_path: Path) -> StatePaths:
    return StatePaths(module_root=MODULE_ROOT, state_root=(tmp_path / "state").resolve())


def pending_interaction(paths: StatePaths, workflow_run_id: str):
    pending = InteractionRequestStore(paths).list_pending_interactions_for_workflow(workflow_run_id)
    assert len(pending) == 1
    return pending[0]


def submit_interaction(paths: StatePaths, request, response_id: str, **value):
    ...
    def read_active_semantic_release(self, request_payload=None):
        return ok_result(
            "read_active_semantic_release",
            {
                ...
                "release_version": "v1",
                "fingerprint": "sha256:release001",
                "active_snapshot": {"snapshot_id": "snapshot_reset_agent"},
                "status": {"database_id": "db:reset_agent"},
            },
        )

    def reset_database(self, request_payload=None):
        payload = dict(request_payload or {})
        self.reset_payloads.append(payload)
        return FakeCorpusAdapter().reset_database(payload)
