...
from mcp_server.tool_catalog import clear_tool_catalog_cache, tool_definitions
...
def _resume_schema() -> dict[str, object]:
    return {
        "type": "object",
...
def _schema_for(name: str) -> dict[str, object]:
    if name == "kernel_continue_resumable_workflow":
        return _resume_schema()
    return _empty_schema()


def test_catalog_exposes_permanent_semantic_control_kernel_names_only(
    monkeypatch,
) -> None:
    ...
    catalog_names = [str(tool["name"]) for tool in catalog]
    ...
    for tool in semantic_control_kernel_tools():
        assert tool["inputSchema"] == _schema_for(str(tool["name"]))


def test_visible_tool_catalog_fetches_kernel_catalog_once(monkeypatch) -> None:
    calls: list[str] = []
    ...
    assert set(PERMANENT_AGENT_TOOL_NAMES) <= visible_names
    assert calls == ["permanent_agent"]
