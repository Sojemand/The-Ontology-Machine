...
from .openai_chat_payload import build_payload, message_to_chat
from .openai_chat_response import parse_response
from .openai_transport import parse_retry_after, request_openai


class OpenAIChatProvider(LLMProvider):
    def __init__(self, api_key: str, model: str, *, base_url: str, timeout: int = 300, provider_name: str):
        self.api_key = api_key
        self.model = model
        self.base_url = base_url.rstrip("/")
        self.timeout = timeout
        self._provider_name = str(provider_name or "openai_chat").strip() or "openai_chat"
    ...
    def generate(self, messages: list[dict[str, Any]], schema: dict[str, Any] | None, max_output_tokens: int, thinking_effort: str) -> str:
        ...
        status_code, headers, response_text = self._request("POST", "/chat/completions", payload=payload)
        if status_code == 429:
