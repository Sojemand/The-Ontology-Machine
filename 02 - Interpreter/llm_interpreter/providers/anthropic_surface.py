...
class AnthropicProvider(LLMProvider):
    def __init__(self, api_key: str, model: str, *, base_url: str, timeout: int = 300, provider_name: str = "anthropic"):
    ...
    def _request(self, method: str, path: str, *, payload: dict[str, Any] | None = None, timeout: int | None = None):
        return request_openai(
            ...
            extra_headers={"x-api-key": self.api_key, "anthropic-version": _ANTHROPIC_VERSION},
        )
    ...
    def generate(self, messages: list[dict[str, Any]], schema: dict[str, Any] | None, max_output_tokens: int, thinking_effort: str) -> str:
        ...
        status_code, headers, response_text = self._request("POST", "/messages", payload=payload)
        if status_code == 429:
