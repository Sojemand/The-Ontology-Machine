"""Workflow helpers for Interpreter edit-contract actions."""
...
def describe(*, paths, module_root) -> dict:
    surfaces = describe_surfaces()
    debug_value = read_surface(DEBUG_CAPABILITIES_SURFACE_ID, paths=paths, module_root=module_root)
...
def read_bundle(*, paths, module_root) -> dict:
    described = describe(paths=paths, module_root=module_root)
    return _bundle_response(described, lambda surface_id: read_surface(surface_id, paths=paths, module_root=module_root))


def read(surface_id: str, *, paths, module_root) -> dict:
    return {"status": "ok", "surface_id": surface_id, "value": read_surface(surface_id, paths=paths, module_root=module_root)}
...
def write(surface_id: str, value: dict, *, paths) -> dict:
    validate_surface(surface_id, value)
    return {"status": "ok", "surface_id": surface_id, "value": write_surface(surface_id, value, paths=paths)}
