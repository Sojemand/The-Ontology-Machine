...
$probeScript = Join-Path ([System.IO.Path]::GetTempPath()) ("interpreter-runtime-check-{0}.py" -f ([guid]::NewGuid().ToString("N")))
...
if (-not ([string]$payload.provenance.encodings).StartsWith($runtimeRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
    $violations += "encodings wird ausserhalb der gebuendelten Runtime geladen: $($payload.provenance.encodings)"
}
